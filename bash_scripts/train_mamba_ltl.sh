#!/usr/bin/env bash
# rev7b: Mamba DPD FL sweep at paper config + last-token loss (matches sliding-inference extraction).
# Usage: bash bash_scripts/train_mamba_ltl.sh <FL> <gpu_id>
...
echo "==== Mamba LTL d=6 d_state=4 FL=${FL} GPU=${devices} (BS=${BS}) ===="
