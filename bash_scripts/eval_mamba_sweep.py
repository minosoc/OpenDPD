"""rev7 sliding eval for Mamba FL sweep."""
import os, sys, json
import numpy as np
import pandas as pd
import torch
...
def main():
    ...
    for FL in [50, 100, 200, 500, 1000]:
        dpd_ckpt_pat = f'save/DPA_200MHz/train_dpd/PA_S_0_M_DGRU_H_8_F_{FL}/DPD_S_0_M_MAMBA_H_6_F_{FL}_P_'
        import glob
        candidates = sorted(glob.glob(dpd_ckpt_pat + '*.pt'))
        if not candidates:
            print(f"SKIP FL={FL}: no ckpt"); continue
        ckpt = candidates[0]
        ...
        print(f"  Mamba FL={FL:5d} | params={n_params} | NMSE={nmse:7.2f} EVM={evm:7.2f} ACLR_L={aclr_l:7.2f} ACLR_R={aclr_r:7.2f} ACLR_AVG={aclr_avg:7.2f}")
        rows.append({'FL': FL, 'params': n_params, 'NMSE': nmse, 'EVM': evm,
                     'ACLR_L': aclr_l, 'ACLR_R': aclr_r, 'ACLR_AVG': aclr_avg})
    df = pd.DataFrame(rows)
    os.makedirs('results/rev7_mamba_fl_sweep', exist_ok=True)
    df.to_csv('results/rev7_mamba_fl_sweep/rev7_mamba_results.csv', index=False, float_format='%.3f')
    print("\nSaved.")
