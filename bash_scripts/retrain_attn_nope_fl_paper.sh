#!/usr/bin/env bash
# Retrain Transformer NO-PE (d_model=6, n_heads=2, d_ff=18, L=1, ~488 params)
# with paper config (eval_match_train_len=0, nperseg=2560 eval).
# Usage: bash bash_scripts/retrain_attn_nope_fl_paper.sh <FL> <gpu_id>
...
echo "==== Transformer-noPE FL=${FL} paper-config retrain on GPU ${devices} (BS=${BS}) ===="
...
echo "==== Transformer-noPE FL=${FL} done ===="
