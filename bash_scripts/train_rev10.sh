#!/usr/bin/env bash
# rev10: Transformer variants @ FL=200, LTL, no PE.
#  variant ∈ { swiglu | gmp }
# Usage: bash bash_scripts/train_rev10.sh <variant> <gpu_id>
...
VARIANT="${1:?usage: $0 <swiglu|gmp> <gpu_id>}"
devices="${2:-0}"
FL=200
...
case "${VARIANT}" in
  swiglu)
    EXTRA="--ffn_type swiglu --d_ff 12"
    ;;
  gmp)
    EXTRA="--use_gmp_stem 1 --gmp_stem_kernel 5 --d_ff 10"
    ;;
  *)
...
echo "==== rev10 transformer ${VARIANT} FL=${FL} GPU=${devices} ===="
python main.py --step train_dpd \
  ...
  --n_heads 2 --use_pos_encoding 0 \
  ${EXTRA} \
  --last_token_loss 1 \
  --eval_match_train_len 0
echo "==== rev10 transformer ${VARIANT} done ===="
