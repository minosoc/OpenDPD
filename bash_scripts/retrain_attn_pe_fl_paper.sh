#!/usr/bin/env bash
# Retrain Transformer with PE (d_model=6, n_heads=2, d_ff=18, L=1, ~488 params)
# with paper config (eval_match_train_len=0, nperseg=2560 eval).
# Usage: bash bash_scripts/retrain_attn_pe_fl_paper.sh <FL> <gpu_id>
...
# Reuse paper-trained DGRU H=8 PA. Length-invariant — same PA OK at any FL.
...
# Batch size scaled to avoid OOM on attention's O(T^2) memory
...
echo "==== Transformer-PE FL=${FL} paper-config retrain on GPU ${devices} (BS=${BS}) ===="
...
echo "==== Transformer-PE FL=${FL} done ===="
